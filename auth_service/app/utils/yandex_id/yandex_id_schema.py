...
from app.utils.yandex_id.avatar_utils import get_avatar_url
...
class User(BaseModel):
    ...
    Note:
        ...
        `first_name`, `last_name`, `display_name`, `real_name` and `sex` fields are available only with `login:info`
        `scope default_phone` field is available only with `login:phone` scope
    ...
    psuid: str = Field(..., description='User ID bassed on client_id and client_secret by Yandex')

    # Only with `with_openid_identity` param
    openid_identities: list[str] | None = Field(None, description='Yandex OpenID identities')

    # Only with `login:email` scope
    default_email: str | None = Field(None, description='Default email of user in Yandex')
    emails: list[str] | None = Field(None, description='List of emails of user in Yandex')

    # Only with `login:avatar` scope
    default_avatar_id: str | None = Field(None, description='Default avatar ID of user in Yandex')
    is_avatar_empty: bool | None = Field(None, description='Flag of default avatar of user in Yandex')

    # Only with `login:birthday` scope
    birthday: datetime | str | None = Field(None, description='Birthday of user in Yandex')

    # Only with `login:info` scope
    first_name: str | None = Field(None, description='First name of user in Yandex')
    last_name: str | None = Field(None, description='Last name of user in Yandex')
    display_name: str | None = Field(None, description='Display name of user in Yandex')
    real_name: str | None = Field(None, description='First and last name of user in Yandex')
    sex: Sex | None = Field(None, description='Sex of user in Yandex')

    # Only with `login:default_phone` scope
    default_phone: Phone | None = Field(None, description='Default phone of user in Yandex')
